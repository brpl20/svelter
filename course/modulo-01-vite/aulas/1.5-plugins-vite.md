...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  1. CONFIG         → Modificar configuração do Vite                 │
│  2. CONFIG RESOLVED→ Configuração final resolvida                   │
│  3. CONFIG SERVER  → Configurar servidor dev (apenas dev)           │
│  4. BUILD START    → Início do build                                │
│  5. RESOLVE ID     → Resolver imports (ex: '@/foo' → '/src/foo')   │
│  6. LOAD           → Carregar conteúdo de módulos virtuais          │
│  7. TRANSFORM      → Transformar código fonte                       │
│  8. BUILD END      → Fim do build                                   │
│  9. CLOSE BUNDLE   → Finalização                                    │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
